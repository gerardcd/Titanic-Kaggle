...
data <- kNN(
  data,
  variable = c("AgeCat", "Fare", "Embarked"),
...
```{r normality_test}
lillie.test(log(data$Fare))
lillie.test(data$Age)
lillie.test(data$Parch)
lillie.test(data$SibSp)
lillie.test(data$FamilySize)
```

### 4.1.2 Tests d'hipòtesis i comparació de distribucions
...
Per a poder aplicar els test d'hiòtesis sobre les variables numèriques, cal demostrar la normalitat de la mitjana mostral i la homoscedasticitat de les mostres. Tot i hem comprovat que no es pot assumir la distribució normal en la població de les variables numèriques, podem assumir pel Teorema Central del Límit que la mitjana mostral d'aquestes sí que tindrà una distribució normal, donat que la mida de la mostra és prou gran. 
