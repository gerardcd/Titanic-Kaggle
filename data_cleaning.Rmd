...
```{r regression}
regVars <- c("Survived", "Fare", "Parch", "SibSp", "Sex", "Embarked", "CabinLetter", "TicketSrc", "AgeCat", "FamilySizeCat", "TitleCat")

train <- data[data$Train <- TRUE, regVars]
...
model <- glm(t_train$Survived ~ t_train$Fare + t_train$Parch + t_train$SibSp + as.factor(t_train$Sex) + as.factor(t_train$Embarked) + as.factor(t_train$CabinLetter) + as.factor(t_train$TicketSrc) + as.factor(t_train$AgeCat) + as.factor(t_train$FamilySizeCat) + as.factor(t_train$TitleCat))

summary(model)
prob <- predict(model, t_test, type="response")
#r <- roc(model, prob, data=train)
#plot(r)

precision <- posPredValue(prob, t_test$Survived, positive="1")
recall <- sensitivity(prob, t_test$Survived, positive="1")
